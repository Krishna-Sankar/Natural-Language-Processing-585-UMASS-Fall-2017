...
def train(examples, stepsize=1, numpasses=10, do_averaging=False, devdata=None):
    ...
    weightsum = defaultdict(float)
    weightsum = Counter(weightsum)
    num_examples = 0
    average_weights = Counter()

    def get_averaged_weights(t):
        w = defaultdict(float)
        for x, y in weightsum.items():
            if y != 0:
                w[x] = y/t
        average_weights = weights - Counter(w)
    ...
    for pass_iteration in range(numpasses):
        ...
        for bow, goldlabel in examples:
            ...
            if predlabel != goldlabel:
                if do_averaging == False:
                    
                    f1 = dict_subtract(features_for_label(bow, goldlabel), features_for_label(bow, predlabel))
                    f1.update((x, y * stepsize) for x, y in f1.items())
                    weights += Counter(f1)
                else:
                    
                    f1 = Counter(dict_subtract(features_for_label(bow, goldlabel), features_for_label(bow, predlabel)))
                    f2 = f1.copy()
                    ...
                    f2.update((x, y * stepsize*(num_examples-1)) for x, y in f2.items())
                    weightsum += f1
        ...
        if devdata and do_averaging:
            print "DEV AVG EVAL:",
            avg_test_acc.append(do_evaluation(devdata, get_averaged_weights(num_examples)))
    ...
    return { 'train_acc': train_acc,
             ...
             'weights': weights if not do_averaging else get_averaged_weights(num_examples) }
...
def plot_accuracy_vs_iteration(train_acc, test_acc, avg_test_acc, naive_bayes_acc):
    ...
    Plot the vanilla perceptron accuracy on the training set and test set
    and the averaged perceptron accuracy on the test set.
    """
    plt.plot(range(1,10), train_acc, color = 'r', label = 'Training Accuracy')
    plt.plot(range(1,10), test_acc, color = 'g',  label = "Test Accuracy")
    plt.plot(range(1,10), avg_test_acc, color = 'b', label = "Average Test Accuracy")
    plt.plot(range(1,10), [naive_bayes_acc]*len(test_acc), color = 'k', label = 'Naive Bayes Accuracy', linestyle = '--')
    plt.ylabel('Accuracy')
    plt.xlabel('Iterations')
    plt.title('Plot Accuracy vs Iteration')
    plt.xticks(range(10))
