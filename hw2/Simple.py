...
def train(examples, stepsize=1, numpasses=10, do_averaging=False, devdata=None):
    ...
    def get_averaged_weights(t):
        w = defaultdict(float)
        print "T in get_averaged_weight(): %d " % t
        for x, y in weightsum.items():
            w[x] = y/t
        average_weights = Counter()
        average_weights = Counter(weights) - Counter(w)
        ...
        for bow, goldlabel in examples:
            ...
            if predlabel != goldlabel:
                ...
                if do_averaging == False:
                    feature = dict_subtract(features_for_label(bow, goldlabel), features_for_label(bow, predlabel))
                    feature.update((x, y * stepsize) for x, y in feature.items())
                    weights = Counter(weights)
                    weights += Counter(feature)
                else:
                    ...
                    weights = Counter(weights)
                    weights += Counter(feature)
                    feature2.update((x, y * stepsize*(num_examples-1)) for x, y in feature2.items())
                    weightsum = Counter(weightsum)
                    weightsum += Counter(feature)
...
def plot_accuracy_vs_iteration(train_acc, test_acc, avg_test_acc, naive_bayes_acc):
    ...
    plt.plot(range(1,10), avg_test_acc, color = 'b', label = "Test Accuracy - Overall Average")
    plt.plot(range(1,10), [naive_bayes_acc]*len(test_acc), color = 'k', label = 'Accuracy- Naive Bayes')
    plt.ylabel('Accuracy-  All Examples')
